...
public class AudioAndroid implements IAudio {
    private AssetManager myAssetManager; //Referencia al asset manager para buscar los .wav
    private SoundPool mySoundPool; //Pool donde gestionar y guardar los sonidos
    ...
    public AudioAndroid(AssetManager assetManager, SoundPool soundPool){
        ...
        mySoundPool = new SoundPool.Builder().setMaxStreams(10).build(); //El soundpool permite 10 "clips" por sonido
    }
    //Crea un nuev sonido de tipo SoundAndroid
    @Override
    public ISound newSound(String file) {
        SoundAndroid s= new SoundAndroid(file, myAssetManager, mySoundPool);
        return s;
    }
    ...
    public void playSound(ISound sound, int loop) {
        ...
        mySoundPool.play(sId,1, 1,1, loop, 1);
    }
